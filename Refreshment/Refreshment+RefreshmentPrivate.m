...
- (void)contentOffsetDidChangeTo:(CGPoint)contentOffset{
    ...
    RefreshmentView *view = self.top;
    if (view && !view.hidden){
        view.visiblePercent = -(contentOffset.y+additionalContentInset.top+(view.adjustable?additionalContentInset.top:0))/view.frame.size.height;
        tryTrigger(scrollView, view);
    }
    view = self.left;
    if (view && !view.hidden){
        view.visiblePercent = -(contentOffset.x+additionalContentInset.left+(view.adjustable?additionalContentInset.left:0))/view.frame.size.width;
        tryTrigger(scrollView, view);
    }
    view = self.bottom;
    if (view && !view.hidden){
